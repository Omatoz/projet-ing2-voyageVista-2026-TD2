...
<div style="max-width: 800px; margin: 3rem auto; padding: 0 1.5rem; font-family: sans-serif;">
    <div class="panier-container" style="background: white; border-radius: 12px; padding: 2.5rem; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.05);">
        ...
        <div style="display: flex; gap: 1rem; margin-top: 2rem;">
            <form method="POST" action="reservation.php" style="flex: 1;">
                <?php if (!isset($_SESSION['user_id'])): ?>
                    <p>Pour confirmer votre réservation, veuillez vous connecter ou vous inscrire.</p>
                    <a href="auth.php?redirect=panier.php" class="btn-panier-main" style="display:block; text-align:center; background-color: #f59e0b; padding: 1rem; color: white; text-decoration:none; border-radius: 8px;">
                        Se connecter / S'inscrire pour payer
                    </a>
                <?php else: ?>
                    <button type="submit" class="btn-panier-main" style="background-color: #4f46e5; width: 100%; border: none; padding: 1rem; color: white; font-weight: 700; border-radius: 8px; cursor: pointer;">
                        Confirmer ma réservation définitive
                    </button>
                <?php endif; ?>
            </form>
